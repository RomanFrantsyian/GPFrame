        ...
        match e {
            ...
            E::Binary(bin) => {
                ...
                let op = match bin.op {
                    ...
                    Lt(_) | Gt(_) | Le(_) | Ge(_) => return self.comparison(bin, sc),
                    // emitter round-trip: `c != 0.0` IS select's raw
                    // condition semantics — translate to c itself
                    Ne(_) => {
                        ...
                        return Err(unsup("!= only supported as `expr != 0.0`".into()));
                    }
