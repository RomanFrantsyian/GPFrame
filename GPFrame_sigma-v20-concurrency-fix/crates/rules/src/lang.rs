...
define_language! {
    pub enum SigLang {
        ...
        "exp" = Exp([Id; 1]),
        "exp2" = Exp2([Id; 1]),
        // Σ v1.6: f32 rounding — REPRESENTABLE so terms carrying it never
        // panic the egraph, but NO rule rewrites through it (rounding kills
        // every algebraic identity: (a+b)+c ≠ a+(b+c) under Rnd32 and even
        // x*1 = x only survives because 1.0 is exact — not worth the risk).
        "rnd32" = Rnd32([Id; 1]),
        "ln" = Ln([Id; 1]),
        "fma" = Fma([Id; 3]),
        "fold" = Fold([Id; 2]),
...
pub fn op_of(n: &SigLang) -> Op {
    ...
    match n {
        ...
        Eq(_) => Op::Eq, Ne(_) => Op::Ne, Exp2(_) => Op::Exp2, Rnd32(_) => Op::Rnd32,
...
pub fn to_egg(t: &Term) -> RecExpr<SigLang> {
    // Σ-ext terms never enter the egraph: no rule may rewrite plugin
    // semantics. Every caller guards on `has_ext()` and SKIPS rewriting
    // (the term passes through untouched); this assert catches a caller
    // that forgot — loudly, not by corrupting.
    assert!(!t.has_ext(),
        "ext term reached the egraph -- rules never rewrite extension \
         semantics; guard with Term::has_ext() and skip rewriting");
...
/// RecExpr → Term, TREE-IFIED: egg's extractor hash-conses, so a RecExpr
/// slot can be shared — including `acc`/`elem` leaves across SIBLING fold
/// bodies, which the Σ v1.2 ownership discipline forbids (v1.5 fission made
/// this reachable). Rebuilding recursively without memoization duplicates
/// every shared reference (the copy_subtree rule: sharing is space, not
/// semantics). No blowup: the extractor's cost function is already TREE
/// cost, so the tree it chose is the tree we pay for. Recursion pushes
/// children before parents, preserving the topological invariant.
pub fn from_egg(e: &RecExpr<SigLang>) -> Term {
    fn go(nodes: &[SigLang], i: usize, b: &mut TermBuilder) -> u32 {
        use SigLang::*;
        match &nodes[i] {
            ...
            Len(k) => b.len_of(k.0),
            Fold([iv, bo]) => {
                let init = go(nodes, usize::from(*iv), b);
                let body = go(nodes, usize::from(*bo), b);
                b.fold(init, body)
            }
            n => {
                let op = op_of(n);
                let kids: Vec<u32> = n.children().iter()
                    .map(|&c| go(nodes, usize::from(c), b)).collect();
                match op.arity() {
                    1 => b.unary(op, kids[0]),
    ...
    let root = go(nodes, nodes.len() - 1, &mut b);
    b.finish(root)
}
