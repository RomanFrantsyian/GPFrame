...
pub fn emit_rust(t: &Term, fn_name: &str, cert: Option<&Certificate>) -> String {
    ...
    fn expr(t: &Term, id: u32, bound: &[bool], var: &dyn Fn(u32) -> String, top: bool) -> String {
        ...
        match n.op {
            ...
            // f32 round-trip: the emitted form the syn door maps back to Rnd32
            Op::Rnd32 => format!("(({}) as f32) as f64", e(n.a)),
            // Σ-ext: emit a call to the plugin's OWN symbol. The emitted
            // file compiles only when linked against a crate providing
            // `fn <name>(f64…) -> f64` with the registered semantics — the
            // certificate's ext tag names exactly which semantics that is.
            Op::Ext1 => format!("{}({})", t.exts[n.b as usize], e(n.a)),
            Op::Ext2 => format!("{}({}, {})", t.exts[n.c as usize], e(n.a), e(n.b)),
    ...
    match cert {
        Some(c) => {
            ...
            if !c.ext_semantics.is_empty() {
                // the claim is equivalence MODULO these plugin semantics —
                // said in the artifact itself, forever
                let _ = writeln!(out, "/// MODULO extension semantics: {}",
                    c.ext_semantics.join(", "));
            }
