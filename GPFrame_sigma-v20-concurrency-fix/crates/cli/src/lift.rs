...
pub enum LiftError {
    ...
    Parse(String),
    /// no `define` with a matching symbol. Carries every symbol that WAS
    /// found in the IR, so a mismatch (rustc-version-specific mangling,
    /// unexpected inlining, a wrong crate-type default on some
    /// toolchain/OS) is self-diagnosing instead of a bare "not found" —
    /// found the hard way (2026-07-18): a report of this error with no
    /// further context on an unreproducible-on-Linux platform gave no
    /// way to tell "wrong name" from "rustc emitted something unexpected"
    /// apart.
    FnNotFound { name: String, found: Vec<String> },
...
impl std::fmt::Display for LiftError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LiftError::FnNotFound { name, found } => {
                if found.is_empty() {
                    write!(f, "no `define` at all in the emitted IR for `{name}` \
                        -- rustc likely failed silently or emitted no functions; \
                        check `RUSTC` and the source compiles standalone")
                } else {
                    write!(f, "no `define` matching `{name}` -- rustc emitted {} \
                        other function(s) instead: {found:?} (if one of these IS \
                        your function under a different symbol, that's a real \
                        mangling/inlining mismatch worth reporting, not a typo)",
                        found.len())
                }
            }
...
fn find_define<'a>(ir: &'a str, fn_name: &str) -> Result<(String, Vec<&'a str>), LiftError> {
    ...
    match hits.len() {
        0 => {
            // collect every symbol rustc DID define, for the diagnosis
            let found: Vec<String> = ir.lines()
                .filter(|l| l.trim_start().starts_with("define"))
                .filter_map(|l| symbol_of(l.trim_start()))
                .map(|s| s.to_string())
                .collect();
            Err(LiftError::FnNotFound { name: fn_name.into(), found })
        }
        1 => {
...
mod fold {
    ...
    enum Term_ {
        Ret(String),
        Br(String),
        CondBr { cond: String, a: String, b: String },
        /// `unreachable` — the end of an inlined panic/assert path. Parsed
        /// (the parser is total over rustc -O1 output) so the REFUSAL can
        /// state the real reason: Σ terms are total, panicking functions
        /// are partial. (MEASURED on statistical 1.0.0: `assert!(len > 1)`
        /// guards inline to call-panic + `unreachable`, and the old parser
        /// reported "no terminator" — a parse error masking the honest
        /// perimeter answer.)
        Unreachable,
    }
    ...
    fn parse_block(rb: &RawBlock) -> Result<Block_, LiftError> {
        ...
        for line in &rb.lines {
            ...
            } else if line.trim() == "unreachable" {
                term = Some(Term_::Unreachable);
            } else if let Some(rest) = line.strip_prefix("br ") {
                // instruction-level metadata (`!prof`, `!llvm.loop`, …) is
                // profile/optimizer annotation, not semantics — drop it
                // (MEASURED: assert-guard branches carry `!prof` at -O1)
                let mut parts = split_top(rest);
                parts.retain(|p| !p.trim().starts_with('!'));
                match parts.len() {
                    1 => {
    ...
    #[allow(clippy::too_many_arguments)]
    fn pass(insts: &[String], seq_ptrs: &HashMap<String, u32>, lens: &[String],
    ...
    pub(super) fn lift_fold(sig: &str, raw: &[RawBlock]) -> Result<Term, LiftError> {
        ...
        // Panic/assert paths (inlined `assert!` → call-panic + `unreachable`)
        // make the function PARTIAL; Σ terms are total, and the extraction
        // gate's claim is over all of μ′ — a lifted term would return a value
        // exactly where the original aborts. Refuse with the audit's own
        // vocabulary: totality guards are refactoring effort, not lifting.
        if let Some(pb) = blocks.iter().find(|bl| matches!(bl.term, Term_::Unreachable)) {
            return Err(refuse(format!(
                "block `{}` ends in `unreachable` -- an inlined panic/assert \
                 path makes the function partial; Sigma terms are total \
                 (totality guards are refactoring effort -- see the audit's \
                 panic-path class)", pb.label)));
        }
        ...
        pass(&entry.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, false, &mut b)?;
        ...
        if let Some(ph) = preheader {
            pass(&ph.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, false, &mut b)?;
        }

        // ---- loop phis: N f64 accumulators; i64 induction machinery.
        // MULTI-ACCUMULATOR (v1.5, fission): each f64 phi becomes its OWN
        // Sigma Fold over the same 0..L iteration space -- classic loop
        // fission, sound because fold bodies are pure and index-blind, so
        // sibling folds commute. Sigma is UNCHANGED: fold_owners, the
        // interpreter, the JIT loop codegen, and the emitter already accept
        // multiple sibling Fold nodes; only this recognizer refused.
        // Soundness precondition, checked per accumulator below: no
        // accumulator's update slice may read ANOTHER accumulator's phi --
        // coupled recurrences (Welford-style) have no fission and refuse.
        let mut acc_phis: Vec<&Phi> = Vec::new();
        for p in &lp.phis {
            match p.ty.as_str() {
                "double" => acc_phis.push(p),
                "i64" => {
        ...
        for p in &acc_phis {
            let (Some(iv), Some(nv)) = (init_of(p), next_of(p)) else {
                return Err(LiftError::Parse(
                    "accumulator phi is not 2-armed entry/loop".into()));
            };
            inits.push(iv);
            nexts.push(nv);
        }
        let single = acc_phis.len() == 1;

        if single {
            // ---- v1.2 single-accumulator path, unchanged
            // init strictly precedes the fold body in the arena (children-first)
            let init_id = operand(&inits[0], &fenv, &mut b)?;
            let acc_id = b.acc();
            fenv.insert(acc_phis[0].dst.clone(), acc_id);

            // ---- loop body
            pass(&lp.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, true, &mut b)?;
            let body_root = *fenv.get(nexts[0].trim_start_matches('%')).ok_or_else(|| refuse(
            ...
            fenv.insert(nexts[0].trim_start_matches('%').to_string(), fold_id);
        } else {
            // ---- v1.5 FISSION path: one Fold per accumulator phi.
            // Classify the body once: f64-dataflow lines (sliced per
            // accumulator) vs recorded-only machinery (int/gep/hints, kept
            // in every slice -- they build no arena nodes).
            let ssa_ops = |rhs: &str| -> Vec<String> {
                rhs.split(|c: char| c.is_whitespace() || matches!(c, ',' | '(' | ')'))
                    .filter_map(|t| t.strip_prefix('%'))
                    .filter(|s| !s.is_empty())
                    .map(str::to_string)
                    .collect()
            };
            let is_machinery_line = |l: &str| -> bool {
                is_void_hint(l) || l.split_once('=')
                    .map(|(_, r)| {
                        let r = r.trim();
                        is_int_machinery(r) || r.starts_with("getelementptr ")
                    })
                    .unwrap_or(true) // no '=': keep; pass() refuses with its own words
            };
            let flines: Vec<(usize, String, Vec<String>)> = lp.insts.iter().enumerate()
                .filter(|(_, l)| !is_machinery_line(l))
                .filter_map(|(i, l)| l.split_once('=').map(|(d, r)| (
                    i,
                    d.trim().trim_start_matches('%').to_string(),
                    ssa_ops(r.trim()),
                )))
                .collect();
            let phi_dsts: Vec<&str> = acc_phis.iter().map(|p| p.dst.as_str()).collect();

            for (k, p) in acc_phis.iter().enumerate() {
                // backward slice of this accumulator's next value over the
                // body's f64 dataflow
                let mut needed: std::collections::HashSet<String> = std::iter::once(
                    nexts[k].trim_start_matches('%').to_string()).collect();
                let mut keep: std::collections::HashSet<usize> =
                    std::collections::HashSet::new();
                for (idx, dst, ops) in flines.iter().rev() {
                    if needed.contains(dst) {
                        keep.insert(*idx);
                        needed.extend(ops.iter().cloned());
                    }
                }
                // fission soundness: the slice must not read a co-accumulator
                for (j, d) in phi_dsts.iter().enumerate() {
                    if j != k && needed.contains(*d) {
                        return Err(refuse(format!(
                            "accumulator `%{}` reads co-accumulator `%{d}` -- coupled \
                             recurrences (Welford-style) have no fission; \
                             cross-accumulator folds are on the roadmap", p.dst)));
                    }
                }
                // per-accumulator env with FRESH Var/invariant nodes: the
                // Sigma v1.2 ownership discipline forbids sharing arena nodes
                // between fold bodies (sharing is space, not semantics --
                // same tree-ification rule as copy_subtree)
                let mut fenv_k: HashMap<String, u32> = HashMap::new();
                for (i, s) in scalars.iter().enumerate() {
                    fenv_k.insert(s.clone(), b.var(i as u32));
                }
                pass(&entry.insts, &seq_ptrs, &lens, &mut fenv_k, &mut ints,
                     &mut geps, false, &mut b)?;
                if let Some(ph) = preheader {
                    pass(&ph.insts, &seq_ptrs, &lens, &mut fenv_k, &mut ints,
                         &mut geps, false, &mut b)?;
                }
                let init_id = operand(&inits[k], &fenv_k, &mut b)?;
                let acc_id = b.acc();
                fenv_k.insert(p.dst.clone(), acc_id);
                let body_k: Vec<String> = lp.insts.iter().enumerate()
                    .filter(|(i, l)| keep.contains(i) || is_machinery_line(l))
                    .map(|(_, l)| l.clone())
                    .collect();
                pass(&body_k, &seq_ptrs, &lens, &mut fenv_k, &mut ints,
                     &mut geps, true, &mut b)?;
                let body_root = *fenv_k.get(nexts[k].trim_start_matches('%'))
                    .ok_or_else(|| refuse(
                        "the accumulator's next value was not recognized as f64 dataflow"))?;
                let fold_id = b.fold(init_id, body_root);
                // post-loop code names the body result: LCSSA rebinding in
                // reverse, once per fissioned fold, in the SHARED env
                fenv.insert(nexts[k].trim_start_matches('%').to_string(), fold_id);
            }
        }
        ...
        // ---- optional LCSSA tail: post-loop f64 applied to the fold
        // result(s). Any reference it makes to OTHER in-body values (a
        // last-iteration live-out) wires Elem/Acc outside the fold -- the
        // fold_owners validator below turns that into a refusal.
        let mut tail_result = nexts[0].clone();
        if let Some(t) = tail {
            pass(&t.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, false, &mut b)?;
            tail_result = t.insts.last().and_then(|l| l.split_once('='))
                .map(|(d, _)| d.trim().to_string())
                .ok_or_else(|| refuse("empty LCSSA tail block"))?;
        }

        // ---- merge. Entry-side arms of tail-processed merges are LLVM's
        // constant-folded value of the tail applied to init -- we
        // deliberately do NOT verify that folding (the lifter is untrusted);
        // mu' samples L = 0, so the gate arbitrates the empty-sequence
        // reading directly.
        if single {
            // the phi's loop-side arm must be the (post-processed) accumulator
            ...
            let entry_side_ok = if tail.is_some() {
                ...
                entry_arm.as_ref().map(|v| *v == inits[0]
                    || matches!((parse_fconst(v), parse_fconst(&inits[0])),
            ...
            // multi-accumulator merges: one f64 phi PER live-out (MEASURED:
            // LLVM may also sink combining arithmetic into the tail --
            // variance's merge phi carries s*s, not s -- so arms mix raw
            // next-values and tail values; resolve each uniformly through
            // the env, where every fissioned fold is already rebound).
            if exit.phis.is_empty() || exit.phis.iter().any(|p| p.ty != "double") {
                return Err(refuse(
                    "exit must merge f64 accumulator live-outs -- non-f64 merge \
                     values are P3"));
            }
            let comp_label = tail.map(|t| t.label.as_str()).unwrap_or(lp.label.as_str());
            for lcssa in &exit.phis {
                let comp_arm = lcssa.inc.iter()
                    .find(|(_, l)| l == comp_label).map(|(v, _)| v.clone());
                let entry_arm = lcssa.inc.iter()
                    .find(|(_, l)| l != comp_label).map(|(v, _)| v.clone());
                let (Some(cv), Some(ev)) = (comp_arm, entry_arm) else {
                    return Err(refuse("exit phi is not 2-armed {entry, computation}"));
                };
                let merged = *fenv.get(cv.trim_start_matches('%')).ok_or_else(|| refuse(
                    "exit phi merges a loop value that is not an accumulator -- \
                     last-iteration live-outs are on the roadmap"))?;
                if tail.is_none() {
                    // no tail: the arm names some fold's next value and its
                    // entry arm must be that fold's init, bit-for-bit
                    let k = nexts.iter().position(|n| *n == cv).ok_or_else(|| refuse(
                        "exit phi merges a loop value that is not an accumulator -- \
                         last-iteration live-outs are on the roadmap"))?;
                    let ok = ev == inits[k]
                        || matches!((parse_fconst(&ev), parse_fconst(&inits[k])),
                                    (Ok(x), Ok(y)) if x.to_bits() == y.to_bits());
                    if !ok {
                        return Err(refuse(
                            "exit phi does not merge {accumulator init, accumulator \
                             next} -- the loop's live-out is not the accumulator"));
                    }
                }
                fenv.insert(lcssa.dst.clone(), merged);
            }
        ...
        pass(&exit.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, false, &mut b)?;
    ...
    fn lift_acyclic(sig: &str, blocks: &[Block_]) -> Result<Term, LiftError> {
        ...
        let succs = |bl: &Block_| -> Vec<usize> {
            match &bl.term {
                ...
                Term_::Ret(_) | Term_::Unreachable => vec![],
            }
        };
            ...
            match &bl.term {
                ...
                Term_::Ret(_) => Err(refuse(
                    "a return block upstream of a merge -- multi-exit regions \
                     are on the roadmap")),
                // cannot occur: lift_fold refuses panic paths before dispatch
                Term_::Unreachable => Err(refuse(
                    "an `unreachable` block upstream of a merge -- inlined \
                     panic/assert path (partial function)")),
            }
        }
        ...
        for &mi in &topo {
            ...
            pass(&m.insts, &seq_ptrs, &lens, &mut fenv, &mut ints, &mut geps, false, &mut b)?;
...
/// A directory unique to this process AND this call. Process id alone is
/// NOT enough: cargo's default test runner executes every test in a
/// binary as a THREAD within one process (identical PID), and a real
/// server handles concurrent requests the same way — two callers sharing
/// one temp dir race on the same `lift_input.rs`/`.ll`, and the loser
/// silently reads back a DIFFERENT function's compiled IR. This is not
/// hypothetical: reported live, 2026-07-18 — `diamond_gate_ease_in_out`
/// asked to lift `ease_in_out` and got back `nested`'s IR, because both
/// tests' concurrent `rustc_emit_ir` calls shared one PID-only dir. Fixed
/// by adding a process-wide atomic counter so every CALL gets its own
/// directory regardless of thread/request concurrency.
pub fn unique_tmp_dir(prefix: &str) -> std::path::PathBuf {
...
pub fn rustc_emit_ir(rs_path: &std::path::Path, fn_name: &str) -> Result<String, String> {
    ...
    let dir = unique_tmp_dir("dge_lift");
