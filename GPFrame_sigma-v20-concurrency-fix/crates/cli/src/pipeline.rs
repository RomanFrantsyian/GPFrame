...
pub struct PipelineOpts {
    ...
    pub lift: bool,
    /// suppress the [1/4]..[4/4] stage chatter on stderr (batch callers set
    /// this so a whole-codebase run doesn't drown its own summary).
    pub quiet: bool,
}

impl Default for PipelineOpts {
    fn default() -> Self {
        Self { eps: false, domain: None, artifacts: "artifacts/o1".into(), lift: false, quiet: false }
    }
}
...
pub struct Certified {
    ...
    pub cost_before: u64,
    pub cost_after: u64,
    /// the PROVEN term behind `code` — carried so batch/in-place callers can
    /// re-verify a rewrite against it without re-deriving the certificate.
    pub term: term::Term,
}
...
pub fn certify(file: &str, name: &str, opts: &PipelineOpts) -> Result<Certified, String> {
    ...
    Ok(Certified {
        code,
        door,
        cost_before: out.cost_before,
        cost_after: out.cost_after,
        term: out.verified.term().clone(),
    })
}
