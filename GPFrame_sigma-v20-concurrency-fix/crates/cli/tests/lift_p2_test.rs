...
/// v1.5 FLIPPED this pin: two INDEPENDENT accumulators fission into sibling
/// folds and must lift (the full gate lives in multiacc_test.rs). The honest
/// refusal moved to COUPLED recurrences, whose update slice reads a
/// co-accumulator and therefore has no fission.
#[test]
fn p2_lifts_multi_accumulator_and_refuses_coupling() {
...
"#, "sum_and_sumsq");
    let t = lift_ll(&ir, "sum_and_sumsq")
        .unwrap_or_else(|e| panic!("independent accumulators must fission: {e}"));
    assert!(t.nodes.iter().filter(|n| n.op == term::Op::Fold).count() == 2,
        "fission must produce two sibling folds");

    let ir = emit("coupled", r#"
pub fn coupled(s: &[f64]) -> f64 {
    let mut m = 0.0;
    let mut q = 0.0;
    for i in 0..s.len() {
        q += (s[i] - m) * s[i];
...
"#, "coupled");
    match lift_ll(&ir, "coupled") {
        Err(LiftError::Refused(m)) =>
            assert!(m.contains("co-accumulator") && m.contains("fission"), "{m}"),
        other => panic!("coupled recurrence must refuse: {other:?}"),
    }
}
