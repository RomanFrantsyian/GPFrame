...
pub fn structural_hash(t: &Term) -> u64 {
    ...
    for n in &t.nodes {
        ...
        match n.op {
            ...
            Op::Var | Op::Elem | Op::Len => h = fnv1a(h, &n.a.to_le_bytes()),
            // Σ-ext: payload slot is a NAME-table index — hash the resolved
            // NAME (so equal terms with permuted ext tables hash equal) and
            // only the true children
            Op::Ext1 => {
                h = fnv1a(h, &sub[n.a as usize].to_le_bytes());
                h = fnv1a(h, t.exts[n.b as usize].as_bytes());
            }
            Op::Ext2 => {
                h = fnv1a(h, &sub[n.a as usize].to_le_bytes());
                h = fnv1a(h, &sub[n.b as usize].to_le_bytes());
                h = fnv1a(h, t.exts[n.c as usize].as_bytes());
            }
            _ => {
                let ar = n.op.arity();
