...
fn apply(op: Op, x: f64, y: f64, z: f64) -> f64 {
    match op {
        ...
        Op::Exp => x.exp(),
        Op::Exp2 => x.exp2(),
        // f32 rounding, widened: exact double-rounding semantics symbol
        Op::Rnd32 => (x as f32) as f64,
        // Σ-ext never reaches apply(): every dispatch site resolves through
        // ext_call (needs the term's name table, which apply doesn't have)
        Op::Ext1 | Op::Ext2 => unreachable!("ext ops dispatch via ext_call"),
        Op::Ln => x.ln(),
        Op::Add => x + y,
        Op::Sub => x - y,
        Op::Mul => x * y,
...
/// Σ-ext dispatch: resolve by name through the registry. Unregistered at
/// EVAL time is a programming error (gates pre-check and refuse honestly
/// before any evaluation) — panic with the exact remedy.
fn ext_call(t: &Term, idx: u32, args: &[f64]) -> f64 {
    let name = &t.exts[idx as usize];
    let d = crate::ext::lookup(name).unwrap_or_else(|| panic!(
        "extension op `{name}` is not registered -- register its plugin \
         before evaluating (gates check this and refuse honestly)"));
    (d.f)(args)
}
...
fn values(t: &Term, env: &Env) -> Vec<f64> {
    ...
    for n in &t.nodes {
        let v = match n.op {
            ...
            Op::Ext1 => ext_call(t, n.b, &[val[n.a as usize]]),
            Op::Ext2 => ext_call(t, n.c, &[val[n.a as usize], val[n.b as usize]]),
            op => {
                let ar = op.arity();
...
fn values_seq(t: &Term, env: &Env, seqs: &[&[f64]]) -> Vec<f64> {
    ...
    for i in 0..n {
        ...
        val[i] = match node.op {
            ...
            Op::Ext1 => ext_call(t, node.b, &[val[node.a as usize]]),
            Op::Ext2 => ext_call(t, node.c,
                                 &[val[node.a as usize], val[node.b as usize]]),
            ...
            // fold_owners tolerates UNREACHABLE orphan binders (fission
            // passes may build nodes dead on their fold's dataflow); their
            // value is never consumed, so 0.0 is a safe placeholder.
            // Outside-REACHABLE binders were already rejected by the
            // validator above.
            Op::Acc | Op::Elem => 0.0,
            Op::Fold => {
                ...
                for it in 0..len {
                    for &j in &owned[i] {
                        ...
                        val[j] = match bn.op {
                            ...
                            Op::Ext1 => ext_call(t, bn.b, &[val[bn.a as usize]]),
                            Op::Ext2 => ext_call(t, bn.c,
                                &[val[bn.a as usize], val[bn.b as usize]]),
                            Op::Len =>
